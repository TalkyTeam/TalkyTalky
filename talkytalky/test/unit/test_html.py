...
def test_assign_ids_call_wild_ch_1():
    ...
    print(project_root)
    in_file = project_root + "/talkytalky/test/exploded_epubs/call_of_the_wild/OEBPS/@public@vhost@g@gutenberg@html@files@215@215-h@215-h-1.htm.html"
    out_dir =  project_root + "/talkytalky/test/temp/util/html/"
    make_dir(out_dir)
...
def test_assign_sentence_ids_peter_rabbit():
    project_root = get_project_root()
    print(project_root)
    in_file = project_root + "/talkytalky/test/exploded_epubs/peter_rabbit/OEBPS/@public@vhost@g@gutenberg@html@files@14838@14838-h@14838-h-0.htm.html"
    out_dir =  project_root + "/talkytalky/test/temp/util/html/"
    make_dir(out_dir)
...
def test_assign_sentence_ids_house_that_jack_built():
    project_root = get_project_root()
    print(project_root)
    in_file = project_root + "/talkytalky/test/exploded_epubs/the_house_that_jack_built_no_images/OEBPS/@public@vhost@g@gutenberg@html@files@12109@12109-h@12109-h-0.htm.html"
    out_dir =  project_root + "/talkytalky/test/temp/util/html/"
    make_dir(out_dir)
